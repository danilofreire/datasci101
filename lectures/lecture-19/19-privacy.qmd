...
:::{.columns}
:::{.column width=50%}
- [EU AI Act]{.alert}: the world's first comprehensive AI law, with risk tiers (banned, high-risk, limited, minimal)
- [US approach]{.alert}: sectoral regulation with state-level initiatives
- [China]{.alert}: state control over content, innovation goals, and mandatory algorithm registration
...
- AI [predicts sensitive information]{.alert} from innocuous data: shopping reveals health, typing speed reveals mood, networks reveal politics
...
- Her mother complained: her daughter was "only in high school"
- Days later the father called back, apologising and confirming the pregnancy
...
- Researchers extracted [verbatim training data]{.alert} from GPT-2, including names, phone numbers and email addresses ([Carlini et al., 2021](https://arxiv.org/abs/2012.07805))
...
2. [Data minimisation]{.alert}: collect only what's necessary
3. [Accuracy]{.alert}: keep data up to date
4. [Storage limitation]{.alert}: don't keep it longer than needed
...
- Legal obligation
- Public interest
- [Legitimate interests]{.alert}: the most contested, used to process without consent (fraud prevention, training AI models)
:::
...
- [[Illinois BIPA](https://www.ilga.gov/legislation/ilcs/ilcs3.asp?ActID=3004)]{.alert}: biometric data, with private right of action
- Many more states are following
:::
...
3. Each sends back only [what it learned]{.alert} (updated weights), not the data
4. The server [averages]{.alert} the updates into a better model
5. The improved model goes back to the devices

**Why it matters for privacy:**

- Your raw data [never leaves your device]{.alert}: the server never sees your messages, photos or browsing history
- Not perfect: updates can still [leak information]{.alert} about the training data
...
:::{.columns}
:::{.column width=55%}
[Synthetic data]{.alert} is data that looks real but isn't ([Jordon et al., 2022](https://proceedings.mlr.press/v133/jordon21a.html)).

**How it works:**

- Train a generative model on real data so it learns the statistical patterns and correlations
- Generate [new, fake data points]{.alert} that look real
- Train your AI on those instead

**Privacy benefits and limitations:**

- No real personal data in the pipeline, so [individuals can't be identified]{.alert}
- You can share it freely for research and collaboration
...
:::{.columns}
:::{.column width=55%}
- In January 2020 the [New York Times](https://www.nytimes.com/2020/01/18/technology/clearview-privacy-facial-recognition.html) revealed Clearview AI had secretly scraped [over 30 billion photos]{.alert} from Facebook, Instagram, LinkedIn and other platforms
- It built a facial recognition database [without anyone's consent!]{.alert} People never knew their faces were in it
- It sold access to over [2,400 US law enforcement agencies]{.alert} and many corporations
- US police used it [nearly a million times]{.alert}

**The legal fallout:**

- The [ACLU sued](https://www.aclu.org/cases/aclu-v-clearview-ai) under [Illinois BIPA]{.alert} (2020). The 2022 settlement [permanently banned]{.alert} sales of the database to private companies nationwide
...
:::{style="background: rgba(230, 57, 70, 0.1); padding: 15px; border-radius: 10px; font-size: 21px; margin-top: 15px;"}
Clearview said the service was only for law enforcement, but gave accounts to [anyone willing to pay]{.alert}, including investors' friends. The ACLU called it a ["perpetual police line-up"](https://www.aclu.org/press-releases/big-win-settlement-ensures-clearview-ai-complies-with-groundbreaking-illinois).
:::
:::
:::
:::
