...
- Quick recap: hallucinations (from Lecture 11)
...
:::{style="font-size: 18px; margin-top: 15px;"}
An ingestion pipeline: collect, parse, chunk, embed, store. Source: [Microsoft (2024)](https://devblogs.microsoft.com/azure-sql/sql-vector-store-data-ingestion-and-retrieval-using-azure-logic-apps/)
:::
:::
:::
:::
...
- LLMs have [context window limits]{.alert} (about 128K–1M tokens): a book fits, a library does not, and long prompts cost more
...
| Study | Finding |
|-------|--------|
| [Lewis et al. (2020)](https://arxiv.org/abs/2005.11401) | Original RAG paper: beat models without retrieval on knowledge-intensive tasks |
| [Shuster et al. (2021)](https://arxiv.org/abs/2104.07567) | Retrieval cut hallucinated replies by [over 60%]{.alert} in dialogue systems |
| [Gao et al. (2024)](https://arxiv.org/abs/2312.10997) | Survey: RAG is a promising fix for hallucination and outdated knowledge |
...
|------|-------|----------|-------------|
| [Gemini Notebook](https://notebook.google/) (ex-NotebookLM) | ✅ Yes | Research, study notes | Multi-source synthesis |
| [ChatGPT + Files](https://chatgpt.com/) | ✅ Free tier | General documents | Easy upload & chat |
...
- Some tools [retrieve]{.alert} the relevant parts (Gemini Notebook, ChatGPT); others read the whole file if it fits (Claude, AI Studio)
...
:::{.column width=45%}
:::{style="text-align: center; margin-top: 40px;"}
[![Gemini Notebook, September 2026](figures/gemini-notebook.png){width="100%"}](#){data-modal-type="image" data-modal-url="figures/gemini-notebook.png"}
...
1. Open <https://aistudio.google.com/> and start a [new chat]{.alert} (Grounding with Google Search off)
...
Gemini reads your whole file and [generates]{.alert} its answer from it. With too many documents to fit, tools add a [retrieval]{.alert} step: that is full RAG
...
- [Research shows]{.alert}: retrieval cut hallucinations by over 60% in one study

- [Watch out for]{.alert}: retrieval failures, lost-in-the-middle, outdated docs

- [No-code tools]{.alert}: Gemini Notebook, ChatGPT with files, etc
