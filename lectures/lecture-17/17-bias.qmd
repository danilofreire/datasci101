...
subtitle: "Lecture 17: Types of Bias and How They Arise"
author:
  - name: Danilo Freire
...
format:
  clean-revealjs:
    ...
    footer: "[Lecture 17](https://danilofreire.github.io/datasci101/lectures/lecture-17/17-bias.html)"
    transition: slide
...
:::{.columns}
:::{.column width=50%}
- Last time: the four [layers of context]{.alert}: instructions, knowledge, memory, tools
- A prompt is one request; a [setup]{.alert} survives the tab closing
- For agents the instructions become a file: [CLAUDE.md]{.alert}, AGENTS.md, SKILL.md
- Memory accumulates on its own, so read it, edit it, use [incognito]{.alert}
- [MCP]{.alert} and connectors: least access, read-only by default, confirm every write
- **Today**: [types of bias]{.alert}, how they arise, and whose values AI encodes
:::

:::{.column width=50%}
:::{style="text-align: center; margin-top: -10px;"}
[![](figures/claude-md-docs.png){width="85%"}](#){data-modal-type="image" data-modal-url="figures/claude-md-docs.png"}

Source: [Anthropic](https://code.claude.com/docs/en/memory)
:::
:::
:::
:::
...
# Well, let's get back to our lecture... 😅 {background-color="#1B3A6B"} 
...
- Held for 30 hours in jail, then released: [wrong person]{.alert}

**What happened?**

- A facial recognition system matched his driver's licence photo to grainy surveillance footage, and was [wrong]{.alert}
- Robert is Black. Research shows facial recognition has [higher error rates]{.alert} for darker-skinned faces

[This was how the system was built]{.alert}
:::
...
# What is bias? 🤔 {background-color="#1B3A6B"}
...
- AI learns from [human-generated data]{.alert}, which contains historical discrimination
- If humans made biased decisions, AI learns those patterns
- AI then [amplifies]{.alert} those biases at scale

**Example: Word embeddings**

- "Man" is to "Doctor" as "Woman" is to... "Nurse"
- The AI picked up [our own sexism]{.alert} from [millions of human texts]{.alert}
:::
...
Neither option is safe. Which [risk]{.alert} would you rather take, and what does that say about what we value?
:::
...
# Types of bias {background-color="#1B3A6B"}
...
:::{.columns}
:::{.column width=55%}
[Past discrimination]{.alert} baked into the training data, even when the data accurately reflects the world at the time

**The Amazon hiring case (2018):**

- Amazon built an AI to screen CVs, trained on [10 years of past hiring]{.alert} decisions
- That hiring was male-dominated
- The AI learned to [penalise CVs mentioning "women's"]{.alert}: "women's chess club captain" and all-women's college both downgraded
- Amazon scrapped the tool

The data was accurate: it reflected Amazon's real hiring. That hiring was biased.
:::
...
:::{.columns}
:::{.column width=55%}
When a group is [underrepresented]{.alert} in training data, the model performs poorly for them

**Example: Voice assistants**

- Trained mostly on American and British accents, male voices (often tech employees), and native speakers
...
- Whoever collects the data determines [who's in it]{.alert}, and developers sample themselves
- "Edge cases" are actually [most of the world]{.alert}
:::

:::{.column width=45%}
:::{style="text-align: center;"}
[![Voice recognition bias](figures/voice-bias.png){width="70%"}](#){data-modal-type="image" data-modal-url="figures/voice-bias.png"}
...
:::{.columns}
:::{.column width=55%}
A [measurable proxy]{.alert} stands in for what you care about, but doesn't work equally for everyone

**Example: ZIP code as a credit proxy**

- Banks can't legally use race in loan decisions, but they can use [ZIP codes]{.alert}
...
- The model works well [on average]{.alert} but poorly for specific groups
...
:::{.columns}
:::{.column width=55%}
The [benchmark dataset]{.alert} used to test a model doesn't represent the population it will be used on
...
:::{.columns}
:::{.column width=55%}
A model works fine where it was built but fails in a [different context]{.alert}
...
- Models trained on hospital data from one region, deployed in others
- X-ray machines, demographics and disease prevalence all differed
...
Bias can appear at [every stage]{.alert}
:::
...
An algorithm's [predictions influence the data]{.alert} it will be trained on in future

**Example: predictive policing**

1. Algorithm predicts hotspots from [past arrest data]{.alert}
...
5. Algorithm grows [more confident]{.alert} in biased patterns
6. Cycle continues...

**Why this is dangerous:**

- The algorithm creates evidence for its own predictions, so bias [compounds over time]{.alert}
...
:::{style="margin-top: 15px; background: rgba(230, 57, 70, 0.1); padding: 15px; border-radius: 10px; font-size: 21px;"}
...
# The hard questions {background-color="#1B3A6B"}
...
- Revealing how the algorithm works lets people game it
- Keeping it secret prevents accountability

**4. Short term vs long term**

- Current data perpetuates historical inequalities
- Ignoring it may reduce accuracy today
:::

:::{.column width=45%}
:::{style="background: rgba(45, 69, 99, 0.1); padding: 15px; border-radius: 10px; margin-top: 30px; font-size: 24px;"}
...
- Group B: 60% repay (historical economic disadvantage)

**Choose ONE approach:**

1. **Same threshold for all**: approve at 70% predicted repayment
   - More Group B rejected

2. **Equal approval rates**: 50% approval in both groups
   - More defaults, higher risk

3. **Equal false rejection rates**: same % of good borrowers rejected in each group
...
- Should the algorithm [correct]{.alert} historical injustice, or just not [perpetuate]{.alert} it?
...
- AI bias is a [technical problem]{.alert}: better data, algorithms, audits
...
- AI learns from human data, so it [inherits human biases]{.alert}

- [Historical bias]{.alert} bakes past discrimination into models

- If a group is [missing from training data]{.alert}, the model fails for them

- Neutral-looking [proxies]{.alert} (ZIP code, test scores) can encode inequality

- The [impossibility theorem]{.alert}: you cannot satisfy all fairness criteria at once

- Choosing a fairness definition is a [values question]{.alert}, not a technical one
