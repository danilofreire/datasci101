...
:::{style="text-align: center; margin-top: 30px; font-size: 20px;"}
[![](figures/cortical-labs.png){width="76%"}](#){data-modal-type="image" data-modal-url="figures/cortical-labs.png"}
...
- Facial recognition matched grainy shop footage to his licence photo, and was [wrong]{.alert}
...
- 2023: [NYC starts enforcing AI hiring audit law](https://www.osc.ny.gov/press/releases/2025/12/dinapoli-new-yorkers-deserve-transparent-hiring-process-when-artificial-intelligence-used-vet-their)
...
- Amazon built an AI to screen CVs, trained on [10 years of CVs]{.alert} sent to the company
- Most came from men, as in the tech industry as a whole
- The AI [penalised "women's"]{.alert} ("women's chess club captain") and two all-women's colleges
...
- Result: [higher error rates]{.alert} for:
  - Black speakers: 35% of words wrong vs 19% for white speakers ([Koenecke et al., 2020](https://www.pnas.org/doi/10.1073/pnas.1915768117))
...
:::{.column width=38%}
:::{style="text-align: center;"}
[![Voice recognition bias](figures/voice-bias.png){width="85%"}](#){data-modal-type="image" data-modal-url="figures/voice-bias.png"}
...
- Banks can't use race in loan decisions, but [ZIP codes]{.alert} can stand in for it
...
- Accuracy fell to [near coin-flip level]{.alert}
:::
...
:::{.columns}
:::{.column width=58%}
A feedback loop: an algorithm's [predictions shape the data]{.alert} it is later trained on
...
5. The algorithm grows [more confident]{.alert}; the cycle repeats
...
:::{.column width=42%}
:::{style="text-align: center;"}
[![Feedback loop: hot-spots policing](figures/feedback-loop.png){width="72%"}](#){data-modal-type="image" data-modal-url="figures/feedback-loop.png"}
...
:::{style="margin-top: 40px; margin-bottom: 40px; font-size: 32px;"}
