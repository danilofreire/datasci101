...
subtitle: "Lecture 14: Documentation and Dataset Governance"
author:
  - name: Danilo Freire
...
format:
  clean-revealjs:
    self-contained: true
    footer: "[Lecture 14](https://raw.githack.com/danilofreire/datasci185/main/lectures/lecture-14/14-documentation-v2.html)"
    transition: slide
...
[![Fun fact of the day!](figures/fun-fact.png){width="60%"}](#){data-modal-type="image" data-modal-url="figures/fun-fact.png"}
...
## The two pillars: Datasheets and Model Cards

:::{style="margin-top: 30px; font-size: 21px;"}
:::{style="text-align: center;"}
[![](figures/ai-model-cards.jpg){width="60%"}](#){data-modal-type="image" data-modal-url="figures/ai-model-cards.jpg"}

Source: [Laura Papworth](https://laurelpapworth.com/datasheets-model-cards/)
:::
...
1. **Motivation**: Why was this created?
2. **Composition**: What's in it?
3. **Collection**: How was it gathered?
4. **Preprocessing**: What was done to it?
5. **Uses**: What should (and shouldn't) it be used for?
6. **Distribution**: How is it shared?
7. **Maintenance**: Who keeps it updated?
:::

:::{.column width=45%}
:::{style="text-align: center;"}
[![Datasheets paper](figures/datasheet.png){width="100%"}](#){data-modal-type="image" data-modal-url="figures/datasheets-paper.png"}
...
## Activity: Read a real datasheet! 📖
...
|-------|--------------|---------|
| GPT-4 | Conversation, assistance | Medical diagnosis |
...
:::{.column width=45%}
:::{style="background: rgba(45, 69, 99, 0.1); padding: 15px; border-radius: 10px;"}
**Real example: [GPT-4 Model Card](https://cdn.openai.com/papers/gpt-4-system-card.pdf)** (edited and summarised)
...
[Clear boundaries protect users and developers alike]{.alert}
:::
:::
:::
:::
...
[Industry trend]{.alert}: Leading companies now report disaggregated metrics in model cards.
:::
:::
:::
:::
...
As AI systems become more complex, a single "model card" isn't enough. **System cards** document the [entire AI system]{.alert}, not just one model.
...
Large language models and image generators are trained on [massive datasets]{.alert} scraped from the web. Understanding how these datasets are created is important to understanding their biases.
...
## Inside a modern dataset card: LAION-5B

:::{style="margin-top: 30px; font-size: 18px;"}
:::{.columns}
:::{.column width=55%}
**[LAION (Large-scale AI Open Network)](https://laion.ai/):**

[LAION](https://laion.ai/) creates open datasets for AI research. Their dataset cards include:

:::{style="background: rgba(45, 69, 99, 0.1); padding: 15px; border-radius: 10px; font-size: 17px;"}
**Metadata for each image:**

- `url`: Where the image was found
- `caption`: Alt-text from HTML
- `similarity`: CLIP score (0-1) measuring how well text matches image
- `nsfw`: NSFW probability score (safety filter)
- `width`, `height`: Image dimensions
- `license`: Metadata license (not image license!)
:::

**What this tells us:**

The dataset knows [very little]{.alert} about the images themselves. It's just URLs and alt-text scraped from the web.
:::
...
```json
{
  "url": "https://example.com/orange-cat.jpg",
  "caption": "An orange cat plays with a red ball",
  "similarity": 0.82,
  "nsfw": 0.01,
  "width": 800,
  "height": 600
...
This is all the documentation that exists for each of the 5.8 billion images. No information about:
...
In 2023, researchers found [child abuse material]{.alert} in LAION-5B. The dataset was temporarily taken offline. Source: [CNN (2023)](https://www.cnn.com/2023/12/21/tech/child-sexual-abuse-material-ai-training-data)
:::
:::
:::
:::
...
This is a simplified example of how you might track where your data comes from. In practice, tools like [DVC](https://dvc.org/) or [MLflow](https://mlflow.org/) do this automatically.
...
## FAIR in practice: What it looks like

:::{style="margin-top: 30px; font-size: 20px;"}
:::{.columns}
:::{.column width=50%}
**A FAIR dataset includes:**

:::{style="font-size: 26px;"}
| Principle | Implementation |
|-----------|----------------|
| **Findable** | DOI: `10.5281/zenodo.123456` |
| **Findable** | Rich metadata in standard format |
| **Accessible** | Available via HTTPS download |
| **Accessible** | No login required for public data |
| **Interoperable** | CSV/JSON with documented schema |
| **Interoperable** | Uses standard vocabularies |
...
:::{.column width=50%}
**Contrast with typical AI datasets:**

:::{style="font-size: 26px;"}
| Principle | Typical AI Dataset |
|-----------|-------------------|
| **Findable** | ❌ No persistent ID |
| **Findable** | ⚠️ Minimal metadata |
| **Accessible** | ⚠️ May disappear |
| **Accessible** | ❌ Often just URLs |
| **Interoperable** | ⚠️ Custom formats |
| **Interoperable** | ❌ No standard vocab |
| **Reusable** | ❌ License unclear |
| **Reusable** | ❌ No provenance |
:::

:::{style="margin-top: 15px; background: rgba(230, 57, 70, 0.1); padding: 15px; border-radius: 10px;"}
**The gap**: Most AI training datasets fail basic FAIR principles. This is a [problem]{.alert}.
:::
:::
:::
:::
...
- Individuals pool data for collective bargaining
- Negotiate with companies as a group
- Like unions, but for data!
...
:::{.column width=45%}
:::{style="text-align: center; margin-top: 20px;"}
[![Data rights](figures/lanier.jpg){width="100%"}](#){data-modal-type="image" data-modal-url="figures/data-rights.png"}
...
4. What if the AI generates poems that compete with yours?
5. Does it matter if you can't tell the difference?
:::
...
:::{style="margin-top: 15px; background: rgba(45, 69, 99, 0.1); padding: 15px; border-radius: 10px;"}
[Trend]{.alert}: Major platforms now [require]{.alert} documentation to publish datasets/models!
:::
:::
:::
:::
...
- `r fa('briefcase')` **Career opportunity**: Documentation skills are increasingly valuable
