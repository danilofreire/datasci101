...
format:
  clean-revealjs:
    self-contained: true
    footer: "[Lecture 25](https://raw.githack.com/danilofreire/datasci185/main/lectures/lecture-25/25-revision.html)"
    transition: slide
...
:::{style="margin-top: 10px; background: rgba(45, 69, 99, 0.1); padding: 12px; border-radius: 10px; font-size: 17px;"}
`r fa('link')` Reward hacking = Goodhart's Law (L5) applied to training. RLHF returns in Lecture 24 as a key approach to alignment.
:::
:::

:::{.column width=45%}
:::{style="text-align: center;"}
[![](figures/anthropic-rlhf.png){width="100%"}](#){data-modal-type="image" data-modal-url="figures/anthropic-rlhf.png"}

RLHF: the method behind ChatGPT

:::{style="margin-top: 15px;"}
[![](figures/bias-variance.gif){width="90%"}](#){data-modal-type="image" data-modal-url="figures/bias-variance.gif"}
:::
:::
:::
:::
...
:::{.column width=45%}
:::{style="text-align: center;"}
[![](figures/king-man-woman-embedding.png){width="100%"}](#){data-modal-type="image" data-modal-url="figures/king-man-woman-embedding.png"}
...
:::{.column width=45%}
:::{style="text-align: center;"}
[![](figures/creativity-accuracy.jpg){width="100%"}](#){data-modal-type="image" data-modal-url="figures/creativity-accuracy.jpg"}
...
- [Brussels Effect]{.alert}: EU rules may become the global default, as happened with GDPR
...
:::{.columns}
:::{.column width=55%}
`r fa('microchip')` **How AI works:** next-token prediction, transformers, embeddings, three learning paradigms, metrics and their limits

`r fa('magnifying-glass')` **How AI handles information:** tokens, prompting (PTCF, CoT), hallucinations, RAG, pipelines and drift

`r fa('scale-balanced')` **Ethics and bias:** bias at every stage, the impossibility theorem, the Optum case, documentation as accountability
:::

:::{.column width=45%}
`r fa('landmark')` **Policy and society:** EU AI Act, GDPR, the privacy paradox, labour markets, the attention economy, environmental costs

`r fa('shield-halved')` **Safety and the future:** concrete safety problems, the alignment problem, multiple plausible futures
