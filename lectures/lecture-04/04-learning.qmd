...
- Different ML tasks require different types of labels (classification, regression, etc.)
...
[![Tweet of the day](figures/tweet-of-the-day.jpeg){width="100%"}](#){data-modal-type="image" data-modal-url="figures/tweet-of-the-day.jpeg"}
:::
...
flowchart TB
    A[📊 Collect Labelled Data] --> B[🔀 Split: Train/Test]
    B --> C[🎯 Choose Algorithm]
    C --> D[🏋️ Train Model]
    D --> E[📈 Evaluate on Test Set]
    E --> F{Good enough?}
    F -->|No| G[🔧 Tune/Change Model]
    G --> D
    F -->|Yes| H[🚀 Deploy]
...
1. **Collect data** with known labels
2. **Split** into training and test sets (never peek at test!)
...
[The test set simulates real-world deployment]{.alert}
:::
:::
:::
...
- [Logistic regression]{.alert}: Classification via sigmoid function
...
:::{.column width=45%}
:::{style="text-align: center;"}
[![Linear regression fits a line](figures/linear-regression.gif){width="100%"}](#){data-modal-type="image" data-modal-url="figures/linear-regression.gif"}
...
- Solution: [Random forests](https://en.wikipedia.org/wiki/Random_forest)—combine many trees
...
:::{.columns}
:::{.column width=55%}
- [Anomaly detection](https://en.wikipedia.org/wiki/Anomaly_detection): Identify rare, unusual observations
- [Learn what "normal" looks like]{.alert}
- [Flag deviations from normal]{.alert}
- Approaches:
  - **Statistical**: Points far from the mean
  - **Distance-based**: Points far from neighbours
  - **Density-based**: Points in low-density regions
  - **Reconstruction**: High error when trying to reconstruct
- Applications:
  - Fraud detection
...
:::{.column width=45%}
:::{style="text-align: center;"}
[![Anomalies are outliers](figures/anomaly-detection.gif){width="100%"}](#){data-modal-type="image" data-modal-url="figures/anomaly-detection.gif"}

Source: [DataScientest](https://datascientest.com/en/isolation-forest-how-do-you-detect-anomalies-in-a-dataset)
:::
:::
:::
:::
...
- No labelled examples—learn from [experience]{.alert}
...
:::{.columns}
:::{.column width=55%}
- [Q-learning](https://en.wikipedia.org/wiki/Q-learning): Learn the value of each action in each state
...
- [Works for discrete state/action spaces]{.alert}
...
- Process:
  ...
  4. Use RL (PPO) to optimise LLM for reward
- Why it works:
