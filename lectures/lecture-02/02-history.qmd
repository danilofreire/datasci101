...
:::{.column width=50%}
- Each [input]{.alert} ($x_i$) (e.g., pixel brightness) is multiplied by a [weight]{.alert} ($w_i$) to increase or decrease the importance of that signal, then summed with a [bias]{.alert} ($b$) (a threshold that determines when the neuron "fires"): $z = \sum x_i \cdot w_i + b$
- The result passes through an [activation function]{.alert} (non-linear transformation) to produce the output: $y = f(z)$
...
We will follow ["The cat sat on the ___"]{.alert} all the way through
:::
:::
:::
:::
...
:::{.columns}
:::{.column width=55%}
- Every word carries a [contacts app]{.alert} listing all the other words
- "The animal didn't cross the street because [it]{.alert} was too tired"
- "it" searches its contacts for "who can get tired?"
- Each word is filed under a name: "animal" under *living creature*, "street" under *place*
- ["animal" is the best match,]{.alert} so "it" pulls in the animal's details
- Every word does this at the same time, for every other word
- You search by name, but you actually wanted the details

:::{style="font-size: 18px; margin-top: 15px;"}
You may meet these called [Query]{.alert} (the search), [Key]{.alert} (the name) and [Value]{.alert} (the details). How well a search matches a name is the [attention score]{.alert}
:::
:::
...
- [Multi-head attention]{.alert}: Multiple attention patterns in parallel (division of labour again!)
...
:::{.columns}
:::{.column width=55%}
- Attention is [looking around]{.alert}: which other words matter here?
- Next comes [thinking it over]{.alert}: what do those connections mean for this word?
- That second step is a [feed-forward network]{.alert} (also called an MLP)
- The two together make one [block]{.alert}, and models stack many:
  - GPT-2 (small): 12 blocks
  - GPT-3: 96 blocks
  - GPT-4 and later: far more
- Each block works at a higher level than the last:
  - Early blocks: spelling and grammar
  - Late blocks: meaning and context
:::

:::{.column width=45%}
:::{style="text-align: center;"}
[![One transformer block](figures/transformer-blocks-02.png){width="45%"}](#){data-modal-type="image" data-modal-url="figures/transformer-blocks-02.png"}
