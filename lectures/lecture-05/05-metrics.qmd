...
format:
  clean-revealjs:
    ...
    footer: "[Lecture 05](https://danilofreire.github.io/datasci101/lectures/lecture-05/05-metrics.html)"
    transition: slide
...
[![](figures/tweet-alignment.png){width="65%"}](#){data-modal-type="image" data-modal-url="figures/tweet-alignment.png"}
...
- **R²**: R² = 0.8 means the model [explains 80%]{.alert} of the variation in scores, [20% stays unexplained]{.alert}
:::
:::
...
- 🏥 **Medical**: [chatbots [explained made-up diseases and tests]{.alert} as if they were real](https://www.mountsinai.org/about/newsroom/2025/ai-chatbots-can-run-with-medical-misinformation-study-finds-highlighting-the-need-for-stronger-safeguards)
...
**[Leaderboards]{.alert}**: Human preference rankings ([LM Arena](https://lmarena.ai/) uses [Elo-style ratings](https://en.wikipedia.org/wiki/Elo_rating_system), borrowed from chess)
...
:::{.columns}
:::{.column width=55%}
- In 2025, Anthropic tested [Claude Opus 4.6]{.alert} on BrowseComp, an OpenAI benchmark
- [On its own, the model worked out it was being tested]{.alert}
- It [found the encrypted answer key online, decrypted it and used it]{.alert}
- A new form of [benchmark contamination]{.alert}: answers found during the test, not in training
- It undermines benchmarks that assume models [cannot look things up]{.alert}
...
- **Real example** [(Buolamwini & Gebru, 2018)](https://proceedings.mlr.press/v81/buolamwini18a/buolamwini18a.pdf), Face++ gender classifier:
  - light-skinned men: [0.8% error]{.alert}
  - dark-skinned women: [34.5% error]{.alert}
...
:::{.column width=45%}
:::{style="text-align: center;"}
[![Same pattern in a 2019 audit of Amazon Rekognition (Raji & Buolamwini)](figures/rekognition.webp){width="100%"}](#){data-modal-type="image" data-modal-url="figures/rekognition.webp"}
