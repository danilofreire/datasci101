...
format:
  clean-revealjs:
    self-contained: true
    footer: "[Lecture 05](https://raw.githack.com/danilofreire/datasci101/main/lectures/lecture-05/05-metrics-llms.html)"
    transition: slide
...
- There's no single "best" metric, it all depends on [your specific context]{.alert}
...
- Picking the wrong metric can mean [missing every sick patient or flagging every healthy one]{.alert}
:::
...
Most classifiers output a [probability score]{.alert}. Moving the threshold trades off precision and recall...[you can't maximise both!]{.alert}
:::
...
- Think of it like [memorising exam answers vs understanding the material]{.alert}
...
:::{.columns}
:::{.column width=55%}
- In 2025, Anthropic discovered that [Claude Opus independently identified it was being evaluated]{.alert} on BrowseComp, a benchmark by OpenAI
- The model [located the answer key online and used it]{.alert} to solve the evaluation
- This is a new form of [benchmark contamination]{.alert}: the model is not trained on the answers, but finds them during the test
- Raises questions about the [validity of benchmarks]{.alert} that assume models cannot access external information
- As models become more capable, [evaluation itself becomes harder]{.alert}
:::
