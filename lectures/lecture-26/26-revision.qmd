...
:::{style="margin-top: 10px; background: rgba(45, 69, 99, 0.1); padding: 12px; border-radius: 10px; font-size: 17px;"}
`r fa('link')` The proxy problem is the most recurring idea in this course: Goodhart's Law (L5), measurement bias (L16), the Optum case (L17), engagement metrics (L22).
:::
:::
...
:::{style="margin-top: 10px; background: rgba(45, 69, 99, 0.1); padding: 12px; border-radius: 10px; font-size: 17px;"}
`r fa('link')` Uses embeddings (L6) to fix hallucinations (L11). But if the documents are biased, the retrieval will be too (L16).
:::
:::
...
:::{style="margin-top: 10px; background: rgba(45, 69, 99, 0.1); padding: 12px; border-radius: 10px; font-size: 17px;"}
`r fa('link')` Concept drift relates to bias (L16): the world changes, the model doesn't. The chatbot failures set legal precedents that motivated regulation (L18).
:::
:::
...
- Reliability [multiplies away]{.alert}: 95% accuracy per step is only ~36% over 20 steps. Long tasks are where agents break
- [Prompt injection]{.alert} becomes dangerous with tools: the [lethal trifecta]{.alert} is private data + untrusted content + external communication
...
## Where bias comes from (Lecture 16)
...
:::{style="margin-top: 10px; background: rgba(45, 69, 99, 0.1); padding: 12px; border-radius: 10px; font-size: 17px;"}
`r fa('link')` Measurement bias = the proxy problem (L3). Textbook example: Optum (L17). Feedback loops relate to concept drift (L14). The impossibility theorem connects to alignment (L25).
:::
:::
...
:::{style="margin-top: 10px; background: rgba(45, 69, 99, 0.1); padding: 12px; border-radius: 10px; font-size: 17px;"}
`r fa('link')` Ties together the proxy problem (L3), measurement bias (L16), and Goodhart's Law (L5). The bias was invisible until disaggregated evaluation (L14).
:::
:::
...
:::{style="margin-top: 10px; background: rgba(45, 69, 99, 0.1); padding: 12px; border-radius: 10px; font-size: 17px;"}
`r fa('link')` Ties together RLHF (L4), Goodhart's Law (L5), bias (L16-17), agents acting on goals (L15), and regulation (L18). This is the question the whole course builds toward.
:::
:::
...
LLMs predict the next word, not the truth. This explains hallucinations, creativity, and sycophancy (Lectures 1, 2, 6, 11)
...
Text, images, and audio all become vectors. RAG, semantic search, CLIP, and multimodal AI all rely on this (Lectures 6, 7, 12)
:::

:::{.column width=50%}
4. [Bias can enter at every stage]{.alert}

Data, labels, training, evaluation, deployment. Feedback loops make it self-reinforcing. The impossibility theorem means fairness requires a values choice (Lectures 3, 14, 16, 17)
...
`r fa('magnifying-glass')` [How AI handles information]{.alert}: tokens, prompting (PTCF, CoT), hallucinations, RAG, agents, pipelines and drift
