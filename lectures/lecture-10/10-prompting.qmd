...
:::{.columns}
:::{.column width=50%}
- Last time: [multi-modal AI]{.alert}
- Images become pixel grids, audio becomes spectrograms
- Both become [embeddings]{.alert}: vectors of numbers
...
- [Today]{.alert}: how do we communicate with these models?
- Same model, [different phrasing]{.alert}, wildly different results
:::
...
[Same model, same headline]{.alert}. Which output is more useful for your pipeline?

:::{style="background: rgba(45, 69, 99, 0.1); padding: 15px; border-radius: 10px; font-size: 18px; margin-top: 15px;"}
**Think about it**: LLMs always give *some* answer. Your job is to [constrain what counts as valid]{.alert}
:::
:::
:::
:::
...
:::{.columns}
:::{.column width=55%}
[Lecture 06](https://danilofreire.github.io/datasci101/lectures/lecture-06/06-language.html): LLMs predict the [next token]{.alert} from probability distributions learned during training.
...
- [Vague prompts activate broad, generic patterns]{.alert}; specific prompts activate [narrow, relevant ones]{.alert}
- The model has no goals or understanding. It produces statistically likely continuations

[A specific prompt narrows the model's options, so you get what you need]{.alert}
:::
...
:::{.columns}
:::{.column width=55%}
From Google's [Gemini for Workspace Prompting Guide](https://services.google.com/fh/files/misc/gemini_for_workspace_prompt_guide_october_2024_digital_final.pdf):
...
**You don't need all four**, says Google, but always include the [task]{.alert}: a verb or command is "the most important component of a prompt"
:::
...
Source: [Google Gemini Prompting Guide](https://services.google.com/fh/files/misc/gemini_for_workspace_prompt_guide_october_2024_digital_final.pdf)
:::
:::
:::
:::
...
:::{.columns}
:::{.column width=55%}
From [Lecture 06](https://danilofreire.github.io/datasci101/lectures/lecture-06/06-language.html): **hyperparameters** are settings [you control]{.alert} that affect model behaviour:
...
- ChatGPT and Gemini [have no temperature slider]{.alert}. Use [Google AI Studio](https://ai.google/studio) or [OpenAI's playground](https://platform.openai.com/playground) instead
- You can also [simulate]{.alert} it in a prompt: ["be extremely precise and factual" (low)]{.alert} or ["be wildly creative and unpredictable" (high)]{.alert}
:::
...
:::{.columns}
:::{.column width=55%}
ChatGPT and Claude carry [hidden text you usually don't see]{.alert} that shapes every response: the **system prompt**.
...
[ChatGPT, Claude, Gemini and Copilot all ship with one]{.alert}
:::
...
Source: [Anthropic system prompt release notes](https://platform.claude.com/docs/en/release-notes/system-prompts)
...
The model processed [millions of instruction-response pairs]{.alert} during training, so it can often tell you what your prompt is missing.
...
[Few-shot works because LLMs are trained on patterned text]{.alert}. Your examples prime the model to continue the pattern
:::
...
**Key insight from [Anthropic's documentation](https://platform.claude.com/docs/en/build-with-claude/prompt-engineering/claude-prompting-best-practices):**

> "Diverse: Cover [edge cases]{.alert} and vary enough that Claude doesn't pick up unintended patterns."
:::
:::
:::
:::
...
:::{.columns}
:::{.column width=55%}
LLMs output any text format. Specifying structure makes outputs [easier to parse and more consistent]{.alert}.
...
[Exact keys and values prevent invented fields]{.alert}
:::
...
:::{.columns}
:::{.column width=55%}
In January 2022, [Wei et al.](https://arxiv.org/abs/2201.11903) showed that worked examples with the reasoning written out [improve accuracy by a wide margin]{.alert} on complex tasks.
...
[Accuracy more than tripled on grade-school maths]{.alert} with just eight worked examples.

**Why does this work?**

Each written step gives the model [more computation]{.alert} and breaks the problem into easier pieces, so it can "check its work" as it goes
:::
...
:::{.columns}
:::{.column width=50%}
[Kojima et al. (2022)](https://arxiv.org/abs/2205.11916) found that [you don't need examples]{.alert}. Adding **"Let's think step by step"** alone triggers reasoning behaviour
...
:::{.columns}
:::{.column width=55%}
[OpenAI's o1](https://openai.com/o1/) (2024) and [DeepSeek-R1](https://github.com/deepseek-ai/DeepSeek-R1) (2025) pioneered **Chain-of-Thought reasoning built into the model itself**. Today GPT-5, Claude and Gemini all have a thinking mode.

**How they differ from standard LLMs:**

- They generate [internal reasoning traces]{.alert} before answering, with no "let's think step by step" needed
...
- CoT prompts become [less necessary]{.alert}
- [PTCF still matters]{.alert}: persona, task, context, format
- [Simplify your prompts]{.alert}: complex reasoning instructions [interfere]{.alert} with the model's native reasoning
:::
...
:::{style="background: rgba(45, 69, 99, 0.1); padding: 15px; border-radius: 10px; font-size: 18px; margin-top: 15px;"}
**Rule of thumb**: with thinking models, focus on [what you want]{.alert} (the task), not [how to think about it]{.alert}. Let the model pick the reasoning strategy
:::
:::
:::
:::
...
[![](figures/agents.webp){width="50%"}](#){data-modal-type="image" data-modal-url="figures/agents.webp"}
...
[Gartner predicts](https://www.gartner.com/en/newsroom/press-releases/2025-06-25-gartner-predicts-over-40-percent-of-agentic-ai-projects-will-be-canceled-by-end-of-2027) that by 2028, at least 15% of day-to-day work decisions will be made autonomously by AI agents
:::
...
:::{.column width=50%}
**The jagged intelligence problem** ([Gans, 2026](https://www.nber.org/papers/w34712)):

Even [perfect prompts sometimes fail]{.alert}.
...
Build a mental [reliability map]{.alert} by testing lots of prompts and tracking what goes wrong

:::{style="background: rgba(230, 57, 70, 0.1); padding: 15px; border-radius: 10px; font-size: 17px; margin-top: 15px;"}
[Always verify critical outputs]{.alert}. Treat them as drafts, not final answers
:::
:::
:::
:::
