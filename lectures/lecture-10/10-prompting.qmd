...
- The same model gives wildly different results depending on [how you phrase your request]{.alert}
:::
...
:::{.columns}
:::{.column width=55%}
From Google's [Gemini for Workspace Prompting Guide](https://services.google.com/fh/files/misc/gemini-for-google-workspace-prompting-guide-101.pdf):
...
It works because it [mirrors how training data is structured]{.alert}: documents have authors (persona), purposes (task), backgrounds (context), and conventions (format)
:::
...
:::{style="background: rgba(45, 69, 99, 0.1); padding: 15px; border-radius: 10px; font-size: 18px; margin-top: 15px;"}
**Pro tip**: when debugging prompts, set temperature to 0 first. Removing randomness isolates the prompt as the cause
:::
:::
:::
:::
...
:::{.columns}
:::{.column width=55%}
ChatGPT and Claude carry [hidden text you never see]{.alert} that shapes every response: the **system prompt**.
...
**Few-shot**: 2-5 examples to show the pattern
...
- They are too similar (the model over-fits to surface patterns)
- The task is [simple enough]{.alert} that examples add noise
- They consume [context window]{.alert} that could hold useful content
...
:::{.columns}
:::{.column width=55%}
In January 2022, [Wei et al.](https://arxiv.org/abs/2201.11903) showed that asking models to display their reasoning [improves accuracy by a wide margin]{.alert} on complex tasks.
...
The model cannot do multi-step reasoning in a single forward pass. Intermediate steps [externalise the reasoning]{.alert}, so the model can "check its work" as it goes
:::
...
:::{.columns}
:::{.column width=55%}
[OpenAI's o1](https://openai.com/o1/) and [DeepSeek-R1](https://github.com/deepseek-ai/DeepSeek-R1) have **Chain-of-Thought reasoning built into the model itself**.
...
**Why this matters**: without the "Thought" steps, the model answers in one shot and gets the date or the price wrong
:::
...
[Gartner predicts](https://www.gartner.com/en/topics/ai-agents) that by 2028, 15% of day-to-day work decisions will be made autonomously by AI agents
:::
