...
2. [Final report]{.alert} (8 December): 5–10 pages. Domain overview, tool evaluation, proposed application
...
- Lecture 10 showed the company's hidden text above your message; now you write your own
  - [Settings > Instructions for Claude]{.alert}: every chat
...
- Rules do not scale ([IFScale, 2025](https://arxiv.org/abs/2507.11538)): the best models follow 68% of 500, the early ones most. Keep them few and first
...
- Both do it: [ChatGPT since 2024](https://openai.com/index/memory-and-new-controls-for-chatgpt/), [Claude Free since 2026](https://support.claude.com/en/articles/11817273-use-claude-s-chat-search-and-memory-to-build-on-previous-context), and [on by default]{.alert}
...
- The file keeps growing: [Chakrabarti (2026)](https://arxiv.org/abs/2608.11095) saw these grow [226%]{.alert}, about five net rules per commit, old ones rarely deleted
...
- Released Aug 2025 by OpenAI Codex, Google, Cursor and others; now ~two dozen tools (Cursor, Gemini CLI, Copilot, Zed), in [60,000+]{.alert} projects
- Claude Code reads it natively; joined the [Agentic AI Foundation](https://www.linuxfoundation.org/press/linux-foundation-announces-the-formation-of-the-agentic-ai-foundation) (Dec 2025)
...
- Pre-built skills run in the web app; Free users can [upload their own]{.alert} too (turn on code execution first)
:::
...
AGENTS.md and CLAUDE.md need software you install. Recognise them, do not memorise them. [They will not be on the quiz]{.alert} because not every student can run them
:::
:::
:::
:::
...
- [Li et al. (2023)](https://arxiv.org/abs/2311.10723) surveyed both: fine-tuned models beat GPT-4 on [classification]{.alert}, but match or trail it on generative tasks
...
- [Least access]{.alert}: only the connectors you need, switched off when done
...
- [Developer Mode](https://developers.openai.com/api/docs/guides/developer-mode) is Plus and above, so the tools layer is where the plans differ most
...
- [Rules]{.alert}: few, and first (IFScale, Lost in the middle)
...
6. Add [one connector]{.alert} and switch off its write tools
...
Source: [Importing and exporting memory, Anthropic Help Centre](https://support.claude.com/en/articles/12123587-import-and-export-your-memory-from-claude)
:::
