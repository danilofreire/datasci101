...
- [Externalities]{.alert}: harms fall on people who never chose the system, like deepfakes of people who never consented
...
- [Extraterritorial reach]{.alert}: sell to the EU, you must comply
- The EU market is large, so [most companies comply globally to avoid fragmentation]{.alert}

**Approach:**

- [Risk-based]{.alert}: rules depend on potential harm
- Horizontal: applies across all sectors
- Technology-neutral: names no specific techniques
:::
...
- Real-time public biometric ID (with exceptions)
...
:::{.columns}
:::{.column width=55%}
- Release weights under a free licence and you are generally [exempt from most GPAI transparency requirements]{.alert}
...
**Example:** [Mistral](https://mistral.ai/) is a French open-source AI company. Its larger models (like Mistral Large) are trained above the systemic risk threshold, so they still face the stricter GPAI rules. Its smaller open-licence models would be largely exempt.
:::
:::
:::
:::
...
- [Securities and Exchange Commission (SEC)](https://www.sec.gov/ai) oversees lending/trading
...
- The next president can [reverse]{.alert} an executive order (and did!)
...
- Now pursues AI governance through privacy legislation
