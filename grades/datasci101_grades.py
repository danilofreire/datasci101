"""Compute final grades for DATASCI 101 from a Canvas gradebook export.
...
Usage:
    uv run --with pandas python datasci101_grades.py path/to/grades.csv
    uv run --with pandas python datasci101_grades.py path/to/grades.csv --compute
    uv run --with pandas python datasci101_grades.py ~/Desktop/grades.csv --compute --output ~/Desktop/final.csv
...
OUTPUT_BASENAME = "datasci101-final-grades.csv"
REPORT_BASENAME = "datasci101-missing-report.txt"
...
def drop_canvas_metadata(df: pd.DataFrame) -> pd.DataFrame:
    """Remove Canvas posting-policy rows and the test-student account."""
...
def inspect_missing(students: pd.DataFrame, item_cols: list[str], report_path: Path, input_name: str) -> None:
    ...
    header = [
        "DATASCI 101 - missing-items report",
        f"Input: {input_name}",
